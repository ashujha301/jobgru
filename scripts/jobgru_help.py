...
CHAT commands (Cursor / Claude Code / Codex — any folder):
  ...
  SendGru rows N   Send LinkedIn connection + note (applied rows; appends Sent add note to H)
...
Pipeline phases (one job-search prompt):
  ...
  SendGru  On-demand only — send connection + note for applied rows you name
           (2 people/row; appends "Sent add note" to column H). Not in pipeline.
...
def help_json() -> dict:
    return {
        ...
        "chat_commands": {
            ...
            "SendGru rows": "On-demand LinkedIn connection + add note (applied rows; appends Sent add note to H)",
